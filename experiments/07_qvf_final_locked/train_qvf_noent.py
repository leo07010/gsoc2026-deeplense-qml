#!/usr/bin/env python
"""QVF NO-ENTANGLEMENT ablation — the decisive control for the module-level
quantum-advantage claim.

Identical to train_qvf_scratch.py QUANTUM mode in every respect EXCEPT the
circuit has NO entangling gates: same AmplitudeEmbedding, same weight shape
(N_LAYERS=4, N_Q=8, 3) = 96 rotation angles applied as per-qubit qml.Rot,
same <Z> readout — the CNOT ring of StronglyEntanglingLayers is removed.

Interpretation grid (vs existing logs, same seed/protocol):
  noent ~= sham      -> entanglement is load-bearing: strongest defensible
                        module-level quantum claim (ent > no-ent > linear).
  noent ~= quantum   -> the 21/22 win comes from probability-marginal
                        structure, not entanglement: claim downgrades to
                        "quantum-inspired structured module".

Reference (Model_II, seed 42, 20 ep): quantum N=500 0.8206 / sham 0.7683;
peak cell N=1500 quantum-sham Delta = +0.1124.
"""
import os, sys
...
def _circuit_noent(amp, weights):
...
class QVFNoEnt(nn.Module):
    def __init__(self, n_classes=3, feat=128):
        super().__init__()
        self.cnn = CNNEncoder(feat)
        self.nae = NAE(feat)
        self.w = nn.Parameter(0.1 * torch.randn(N_LAYERS, N_Q, 3))
        self.head = nn.Sequential(nn.LayerNorm(N_Q), nn.Linear(N_Q, n_classes))

    def forward(self, x):
        amp = self.nae(self.cnn(x))
        z = torch.stack(_circuit_noent(amp, self.w), dim=-1).to(amp.dtype)
        return self.head(z)
...
def main():
    ...
    print(f"[INFO] QVF-NOENT (96 angles, no CNOT) | data={os.path.basename(args.data)} "
    ...
    if args.n_per_class > 0:
        ...
        print(f"[INFO] subsampled to N={args.n_per_class}/class", flush=True)
    if args.smoke:
    ...
    for ep in range(args.epochs):
        ...
        print(f"[ep {ep+1:02d}] val AUC={m['auc']:.4f} acc={m['acc']:.4f}", flush=True)
    per_s = (" ".join(f"{cn[i]}={best_per[i]:.4f}" for i in range(len(cn)))
             if best_per is not None else "n/a")
    print(f"\n[DONE-QVF-NOENT] {base} N={args.n_per_class or 'full'} "
